...
class CoreDataStack {
    ...
    private func managedObjectContextObjectsDidChanged(notification: Notification) {
        ...
        if let inserts = userInfo[NSInsertedObjectsKey] as? Set<NSManagedObject>,
           inserts.count > 0 {
            LoggerService.shared.printLogs(text: "Inserted objects count: \(inserts.count)")
        }

        if let updates = userInfo[NSUpdatedObjectsKey] as? Set<NSManagedObject>,
           updates.count > 0 {
            LoggerService.shared.printLogs(text: "Updates objects count: \(updates.count)")
        }

        if let deletes = userInfo[NSDeletedObjectsKey] as? Set<NSManagedObject>,
           deletes.count > 0 {
            LoggerService.shared.printLogs(text: "Deleted objects count: \(deletes.count)")
        }
    }
    ...
    func printDataBaseStatistics() {
        container.viewContext.perform {
            do {
                ...
                LoggerService.shared.printLogs(text: "Channels count: \(channelsCount)" + "\n" + "Total messages count: \(messagesCount)")
            } catch {
